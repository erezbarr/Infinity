...
public class SqlCompanyToContactJunction implements SqlDBResource {
	private DatabaseManagement db;
	private final String TABLENAME = ResourceType.COMPANY_TO_CONTACT.toString();

	
	public SqlCompanyToContactJunction(DatabaseManagement databaseManagement) {
	...
	public synchronized int add(Map<String, Object> resourceData) {
		try {
			return db.createRow(DriverUtils.createSQLAddRow(TABLENAME, 
															...
															resourceData.get(DriverUtils.CONTACT_ID).toString()),
								TABLENAME,
								DriverUtils.ROW_ID);	
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int edit(Map<String, Object> resourceData) {
		...
		return Integer.parseInt(resourceData.get(DriverUtils.ROW_ID).toString());
	}
