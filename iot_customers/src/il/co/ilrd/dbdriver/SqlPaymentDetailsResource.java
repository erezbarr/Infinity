...
public class SqlPaymentDetailsResource implements SqlDBResource {
	private DatabaseManagement db;
	private final String TABLENAME = ResourceType.PAYMENT_DETAILS.toString();
	
	public SqlPaymentDetailsResource(DatabaseManagement databaseManagement) {
	...
	public List<Map<String, Object>> get(KeyType key, Object value) {
		...
		case BILLING_ADDRESS:
			keyToSearch = DriverUtils.BILLING_ADDRESS;
			break;
		default:
			return null;
		}
	...
	public synchronized int add(Map<String, Object> resourceData) {
		try {
			return db.createRow(DriverUtils.createSQLAddRow(TABLENAME, 
															 DriverUtils.NULL, 
															 resourceData.get(DriverUtils.CC_ID).toString() , 
															 resourceData.get(DriverUtils.BILLING_ADDRESS).toString()),
							TABLENAME,
							DriverUtils.PAYMENT_ID);	
		} catch (SQLException e) {
			e.printStackTrace();
			return 0;
		}
	...
	public int edit(Map<String, Object> resourceData) {
		...
		editList.add(DriverUtils.BILLING_ADDRESS);
		try {
		...
		return Integer.parseInt(resourceData.get(DriverUtils.PAYMENT_ID).toString());
	}
