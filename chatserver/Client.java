...
public class Client implements Runnable{
	...
	public void run() {
		...
		user.runCheckInputThread(); //UnknownHostException, ClassNotFoundException, IOException 
	}
	...
	private void registerToChat(String userName) throws UnknownHostException, IOException, ClassNotFoundException {
		...
		buffer.put(message.toByteArray(message));
		keepListening = true;
		runPrintMessages();
		buffer.clear();
		socket.write(buffer);
		...
		Runnable messagePrinting = new Runnable() {
			...
			public void run() {
				try {
					while(keepListening) {
						socket.read(buffer);
						ServerMessage retMessage = (ServerMessage) message.toObject(buffer.array());
